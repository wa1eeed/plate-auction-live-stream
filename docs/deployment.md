...
`settingsPersisted()` تقول أمُفعَّلٌ الحفظ أم لا، وتُقرأ في الإدارة ليُقال لمن
يضبط أيبقى ضبطه بعد النشرة أم لا.
...
- **docker compose** → `volumes: ['plate-data:/app/data']` وحجمٌ مسمّى في
  `volumes:` أعلى الملفّ.
