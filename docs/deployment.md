...
PLATFORM_DATA_DIR=/data                     # مجلّد دائم — بلا ضبطٍ لا يبقى ضبط الإدارة
```

`PLATFORM_DATA_DIR` **يجب أن يُربط بمجلّدٍ دائم** (حجم في Coolify/Docker).
المخزن في الذاكرة، فبلا هذا المتغيّر تعود الهويّةُ والصفحاتُ والعمولةُ
وإعداداتُ الدفع افتراضيّةً مع كلّ نشرة — وهو ما لا يُكتشف إلّا بعد النشر
فيُظنّ الحفظُ نفسه معطّلًا. التفصيل في
[الهويّة والأرشفة وصفحات المنصّة](brand-and-pages.md#6-الثبات-بعد-إعادة-النشر).
