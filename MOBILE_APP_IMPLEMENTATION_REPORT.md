...
**والبناءان نجحا فعلًا** على أجهزة GitHub — لا على هذا الجهاز: لا Xcode فيه
ولا Android SDK ولا Java. انظر القسم ٧.
...
## ٧. ما جُرّب وما لم يُجرَّب — بصراحة

### جُرّب ونجح

| البند | الدليل |
| --- | --- |
| **بناء أندرويد** | تشغيل `35484607863` — نجح، ومخرجاته ٤ ميغابايت |
| **بناء iOS** | تشغيل `35484609356` — نجح على `macos-15` |
| توليد المشروعين ومزامنتهما | `cap sync` للمنصّتين |
| صحّة ملفّي الارتباط | يردّان 404 بلا ضبط، وJSON صحيحًا معه |
| البوّابة كاملةً | `tsc` · `lint` · ٤١٢ وحدة · ١٣٢/١٣٢ متصفّح |

والبناءان **بلا توقيع**: يُثبتان أنّ المشروع يُترجم ويُربط ويُحزَم — وهو ما
يُراد قبل شراء حساب. ولا يُنتجان ما يُثبَّت على جهاز.

### لم يُجرَّب بعد

| البند | السبب |
| --- | --- |
| `.ipa` و`.aab` **موقَّعان** | يلزمان حسابَي آبل وجوجل وشهادةَ توقيع |
| وصول دفعٍ حقيقيّ | يلزم حساب Firebase ومفتاح APNs وجهازٌ حقيقيّ |
| الروابط الكونية | تتحقّق بعد التوقيع والتثبيت على جهاز |

**ولا يقول هذا التقرير إنّ التطبيق جاهزٌ للمتجر.** جاهزٌ ما كُتب، ومُثبَتٌ أنّه
يُبنى؛ والتوقيع والاختبار على جهازٍ حقيقيّ عملٌ يبدأ من القسم التالي.

### وثلاثة أخطاءٍ كشفها التشغيل الحقيقيّ

لم يظهر واحدٌ منها في الفحص المحلّيّ:

1. **`mobile-android.yml` كان ملفًّا غير صالح** — `secrets` لا يُقرأ في `if`،
   فيُرفض الملفّ كلّه ويسقط على كلّ دفعة.
2. **مشروع iOS بلا مخطَّط** — Capacitor يترك Xcode يصنعه في `xcuserdata`،
   وهو لا يدخل المستودع. فلا يجد `xcodebuild` هدفًا يبنيه.
3. **نسخةُ pnpm محدَّدة مرّتين** — `version: 10` في الأكشن و`9.15.0` في
   `package.json`، والأكشن يرفض التعارض. و`ci.yml` بجانبهما لا يقع فيه —
   فبان أنّ الملفّين كُتبا بلا قراءة العامل بجوارهما.
