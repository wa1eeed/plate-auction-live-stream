 ...
 *
 * وهي **مفعّلة افتراضيًا**: المزايدة تُسمع قبل أن تُرى، ومن انتظر مزادًا يعرف
 * أنّ النبرة تسبق العين إلى الشاشة. وصمتٌ افتراضيّ يعني أنّ أحدًا لا يسمعها
 * أصلًا — والقلّة وحدها تفتح الإعدادات لتُشعل ما لا تعرف بوجوده.
 *
 * والمتصفّح يمنع التشغيل قبل أوّل تفاعل على أيّ حال، فلا صوت يفاجئ زائرًا لم
 * يلمس الصفحة بعد. ومن أطفأها يُحفظ اختياره فلا يُنقض عليه.
 */
...
export function useSound() {
  ...
  /*
   * تبدأ الحالة `false` ثمّ تُصحَّح بعد التركيب.
   *
   * الخادم لا يقرأ `localStorage`، فلو بدأت `true` اختلف ما رسمه عمّا رسمه
   * المتصفّح وسقط الترطيب. والقراءة هنا تجعل الافتراضيّ تشغيلًا: «off» وحدها
   * تُسكت، وغيابُ القيمة صوتٌ.
   */
  useEffect(() => {
    try {
