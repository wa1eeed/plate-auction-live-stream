...
/**
 * سببُ تعطيل الحساب — **بطلب صاحبه، أو بقرار الإدارة**.
 *
 * والتفريق ليس ترفًا: ما عُطّل بطلب صاحبه تُعيده الإدارةُ متى طلب، وما
 * عُطّل لمخالفةٍ لا يُعاد بطلبٍ منه. ولو جُمعا في راية `disabled` واحدة
 * لَصار قرارُ الإدارة وطلبُ المستخدم شيئًا واحدًا في السجلّ.
 */
export type DisabledReason = 'self_requested' | 'admin'

export const DISABLED_REASON_LABELS: Record<DisabledReason, string> = {
  self_requested: 'عُطّل بناءً على طلب صاحبه',
  admin: 'عطّلته الإدارة',
}
...
export type User = {
  ...
  /**
   * لحظةُ التعطيل — و`null` تعني حسابًا عاملًا.
   *
   * **والتعطيل ليس حذفًا.** آبل تشترط أن يستطيع المستخدم حذف حسابه من داخل
   * التطبيق، ومحوُ صفّه محوٌ لتاريخٍ ماليّ تقابله صفقاتٌ وقيودُ محفظةٍ
   * وفواتير — فتصير طلباتٌ بلا مشترٍ وقيودٌ بلا صاحب. فيُعطَّل الحساب:
   * لا يدخل صاحبُه ولا يُتعامل به، ويبقى ما يشير إليه سليمًا.
   */
...
export type PublicBid = {
  ...
  isMine: boolean
  /**
   * مهلة سداد صاحب هذه المزايدة إن رست عليه ولم يسدّد — للبائع ولصاحبها وحدهما.
   *
   * الكشف عامّ وأسماؤه مقنّعة، وحالُ سدادِ رجلٍ شأنه وشأن من يبيعه. فمن لا
   * يعنيه الأمر لا يصله الحقل أصلًا، لا يصله ويُخفى بالتنسيق.
   */
  paymentDueAt: string | null
}
...
export type AccountOffer = Offer & {
  ...
  /** الرقمُ المرجع على الإعلان — به يُقاس بُعدُ العرض عمّا يريده البائع */
  listingAsk: Halalas
  /**
   * أهو ثمنٌ مطلوب أم حدٌّ أدنى؟ — وبينهما فرقٌ في المعنى لا في الرقم.
   *
   * فإعلانُ التفاوض لا ثمنَ له، وإنّما `minimumOffer` حدٌّ لا يُقبل دونه،
   * ولا عرضَ يأتي تحته أصلًا — فتسميتُه «المطلوب» تجعل كلّ عرضٍ يبدو فوق
   * المراد، والبائعُ لم يُعلن مرادًا. والبيعُ المباشر بعكسه: `price` ثمنٌ
   * مطلوب، وما دونه نقصٌ يُوازن.
   */
  listingAskKind: 'ask' | 'floor'
  /**
  ...
  isHighest: boolean
  /**
   * سجلُّ الطرف الآخر في المنصّة — لا شارةُ «موثّق» مبهمة.
   *
   * والبائعُ حين يوازن بين عرضين يسأل سؤالًا واحدًا: أيُّهما يدفع؟ فيُقال
   * له ما نملكه فعلًا — كم صفقةً أتمّها هذا، وكم واحدةً سقطت بعدم سداده —
   * لا وسامٌ تمنحه المنصّةُ بلا معنى خلفه.
   */
  counterpartRecord: { settled: number; defaulted: number }
}
 ...
 */
/**
 * اشتراك دفعٍ لجهازٍ واحد — لا لحسابٍ ولا لشخص.
 *
 * `endpoint` عنوانٌ يولّده **متصفّح المستخدم** عند خادم الدفع التابع لصانعه
 * (جوجل لكروم، وآبل لسفاري)، ومعه مفتاحان يُشفَّر بهما ما يُرسل. وهو الطريق
 * الوحيد إلى جهازٍ مغلقٍ أو متصفّحٍ مقفل — بلا حفظه لا سبيل إليه.
 *
 * وليس فيه ما يدلّ على صاحبه: سلسلةٌ عشوائية لا تُعرف إلّا بربطنا نحن لها
 * بحسابٍ عندنا. ومن فتح المنصّة على جوّاله وحاسوبه فله عنوانان يُرسَل إليهما.
 *
 * **ويعيش في الذاكرة كما يعيش صاحبه.** كان في النيّة حفظُه في ملفٍّ ليبقى بعد
 * النشر، ثمّ تبيّن أنّ ذلك لا ينفع: معرّفات المستخدمين تُولَّد عشوائيًّا في كلّ
 * إقلاع، فالاشتراك المحفوظ يعود مربوطًا بحسابٍ لم يعد موجودًا. فالهويّة
...
/**
 * ما يخترق وضع التركيز — **ثلاثةٌ لا أكثر**.
 *
 * إشعارات iOS تُكتم في وضع التركيز والنوم و«عدم الإزعاج»، فلا يراها صاحبها
 * حتى يفتح جهازه. و`time-sensitive` تخترق ذلك — ويبقى له أن يُطفئها من
 * إعدادات التطبيق، فلا إكراه فيها.
 *
 * **والمعيار ضيّقٌ عمدًا**: أن يخسر مالًا أو فرصةً إن لم يرها **خلال دقائق**.
 * وهو أضيق من `URGENT_NOTIFICATIONS`: تلك تُدفَع إلى الجهاز، وهذه تُوقظه.
 *
 ...
 * عاجلٌ يساوي لا شيء، ويُطفئها صاحبها كلَّها فيخسر الثلاثة معها.
 *
 * وتحتاج استحقاق `com.apple.developer.usernotifications.time-sensitive` في
 * `ios/App/App/App.entitlements` — وبدونه تتجاهلها أبل بلا خطأ.
 */
export const TIME_SENSITIVE_NOTIFICATIONS: readonly NotificationType[] = [
  'outbid',
  'payment_due_soon',
  'payment_overdue',
]
...
export type PageSettings = {
  ...
  terms: EditableDoc
  /**
   * سياسة الخصوصية — **يشترطها المتجران** لا نكتبها تطوّعًا.
   *
   * App Store وGoogle Play يطلبان رابطًا عامًّا لها قبل قبول أيّ رفع، وهي
   * أيضًا ما يُقاس به إقرارُ البيانات في `PrivacyInfo.xcprivacy` ونموذج
   * «أمان البيانات» في Play. فاختلافُها عنهما سببُ ردٍّ في المراجعة.
   */
  privacy: EditableDoc
  /**
...
export const DEFAULT_PAGE_SETTINGS: Omit<PageSettings, 'updatedAt' | 'updatedByAdminId'> = {
  ...
  privacy: {
    ...
    intro:
      'ما نجمعه، ولماذا، ومن يراه. والمنصّة لا تبيع بياناتك ولا تشاركها لإعلانٍ ولا تتبّعك خارجها.',
    published: true,
    sections: [
      {
        heading: 'ما نجمعه',
        body:
          'بريدك الإلكتروني واسمك المعروض ومعرّفك العلنيّ — وهي ما يقوم به الحساب. ورقم جوّالك ومدينتك وحساباتك في التواصل اختيارية تضيفها إن شئت. وبيانات حسابك البنكي تُطلب عند طلب صرفٍ لا قبله.\n\nوما تفعله في المنصّة يُحفظ لأنّه سجلّ مالك: مزايداتك وعروضك وصفقاتك وعرابينك وحركات محفظتك.',
      },
      {
        heading: 'ما لا نجمعه',
        body:
          'لا موقعك الجغرافي، ولا جهات اتصالك، ولا صورك، ولا كاميرا ولا ميكروفون — والتطبيق لا يطلب هذه الأذونات أصلًا.\n\nولا نستعمل مُعرّفًا إعلانيًّا، ولا نضع فيك أدوات تتبّعٍ تتبعك إلى مواقع غيرنا، ولا نبيع بياناتك لأحد.',
      },
      {
        heading: 'أرقام البطاقات لا تصل إلينا',
        body:
          'السداد يمرّ ببوّابة دفعٍ مرخّصة، وتُدخل بيانات بطاقتك عندها لا عندنا. والذي يصلنا مرجعُ العملية ونتيجتها ومبلغها — لا رقم بطاقةٍ ولا رمزها.',
      },
      {
        heading: 'من يرى ماذا',
        body:
          'ما تنشره علنًا — لوحتك وسعرها ومزايداتك عليها — يراه كلّ زائر. واسمك المعروض ومعرّضك العلنيّ يُريان في صفحة معرضك.\n\nورقم جوّالك **لا يظهر لأحد** إلّا للطرف الآخر في صفقةٍ تمّت بينكما، وللإدارة عند فصل اعتراض. وبريدك لا يُعرض لمستخدمٍ آخر أبدًا. والسعر الاحتياطيّ الذي تضعه لا يبلغ المزايدين.',
      },
      {
        heading: 'الإشعارات',
        body:
          'إن سمحت بالإشعارات حُفظ رمزُ جهازك مرتبطًا بحسابك — وهو ما يوصل إشعار المزايدة إليك أنت دون غيرك. ويُحذف الرمز متى منعتَ الإذن أو حذفتَ التطبيق أو خرجتَ من حسابك.',
      },
      {
        heading: 'أين تُحفظ وكم تبقى',
        body:
          'البيانات في قاعدةٍ على خادمٍ نديره، والاتّصال مشفَّرٌ كلُّه. وكلمات المرور لا تُحفظ كما هي بل تُخزَّن مُلبَّسةً لا تُستعاد.\n\nوسجلّات المال — الصفقات والقيود والفواتير — تبقى ما دام يلزم لإثبات حقٍّ أو للامتثال الضريبيّ، ولو أُغلق الحساب.',
      },
      {
        heading: 'حقوقك',
        body:
          'لك أن تطّلع على بياناتك وتصحّحها من صفحة حسابك في أيّ وقت. ولك أن تطلب حذف حسابك — فيُحذف ما لا يلزم بقاؤه لسجلٍّ ماليّ أو نظاميّ.\n\nوللطلب أو الاستفسار: راسلنا من صفحة «من نحن».',
      },
      {
        heading: 'التغيير',
        body:
          'إن تغيّرت هذه السياسة نُحدّث هذه الصفحة. واستمرارُ استعمالك المنصّة بعد التغيير موافقةٌ عليه.',
      },
    ],
  },
  howItWorks: {
