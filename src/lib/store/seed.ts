...
export function seedDatabase(db: MemoryDatabase): void {
  ...
  const faqSeed: Omit<FaqItem, 'id' | 'createdAt' | 'updatedAt'>[] = [
    ...
    },
    {
      ...
      answer:
        'تسدّد عبر المنصّة — بطاقة أو حوالة بنكية أو رصيد محفظتك — ويُخصم عربونك من المبلغ فلا تدفعه مرّتين. ويبقى ما سدّدته **محفوظًا لدى المنصّة لا يصل البائع**، حتى ينقل اللوحة باسمك عبر القنوات الرسمية ويرفع إثبات النقل، وتتحقّق الإدارة منه ثم تحوّل المبلغ إليه. ولك أن تفتح اعتراضًا في أيّ وقت قبل التحويل فيتوقّف حتى تفصل الإدارة، وإن لم تُنقل الملكية عاد إليك مالك.',
      category: 'payment',
      sortOrder: 9,
      published: true,
      showOnSaleTypes: ['auction', 'fixed', 'offers'],
  ...
  db.offers.push(
    ...
    /* عرضٌ سام عليه البائع — به يُرى الخيطُ من طرفيه */
    {
      id: newId('ofr'),
      listingId: listings[5].id,
      buyerId: users[1].id,
      ...
      message: null,
      status: 'countered',
      counterAmount: riyalsToHalalas(38_000),
      counterMessage: 'أقلّ ما أبيع به.',
      counterAt: iso(-1_200_000),
