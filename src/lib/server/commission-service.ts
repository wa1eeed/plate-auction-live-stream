/**
 * عمولة المنصّة وضريبتها — ولكلّ طرفٍ موضع اقتطاع.
 *
 * عمولة **البائع** تُقتطع من عائد بيعه عند الإفراج (`releaseOrderEscrow`):
 * المال في يد المنصّة حينها، فتُخصم قبل أن يُقيَّد العائد ولا تُطلب منه بعده.
 *
 * وعمولة **المشتري** تُقتطع عند التحصيل، وقد سدّدها ضمن ما سدّد — فتُقيَّد
 * إيرادًا محصَّلًا بلا خصمٍ ثانٍ من محفظته. فإن لم تُجمَّد ضمن سداده ولم يكفِ
 * رصيدُه، قُيّدت **مستحقًّا** يظهر للإدارة للتحصيل: صفقةٌ تُعلَّق لأنّ محفظة
 * صاحبها لا تكفي عمولتها تعاقب على أمرٍ ليس فيه نزاع.
 *
 * (كان هذا الرأس يقول إنّ «قيمة اللوحة لا تمرّ بالمنصّة» — وصفُ نموذجٍ سابقٍ
 *  على الضمان، ولم يعد صحيحًا لأيٍّ من الطرفين.)
 */
